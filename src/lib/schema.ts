...
export function omitOptional<T extends ZodObject<any>>(schema: T): ZodObject<Omit<T["shape"], OptionalKeys<T["shape"]>>> {
  const optionalKeys = Object.keys(schema.shape).filter(
    key => schema.shape[key].isOptional(),
  );

  const omitObject = optionalKeys.reduce((acc, key) => {
    acc[key] = true;
    return acc;
